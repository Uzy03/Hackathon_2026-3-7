...
SYSTEM_PROMPT: str = (  # 感情に応じた変換強度の使い分けを指示するセクション
    ...
    '{ "aggressionScore": float, "converted": string }'
)  # 文字列定義をここで閉じる
...
FEW_SHOT_EXAMPLES: str = (  # 出力精度を高めるための具体例（Few-shot prompting）を定義する
    ...
    '出力: { "aggressionScore": 0.8, "converted": "【顧客対応・態度】担当者の態度に関するご指摘。および、担当者変更のご要望。" }\n'  # 陰湿な感情を除去し，事務的要約を行う（カテゴリ付与）
    "\n"
    "【変換例2：事実の抽出と攻撃的な言葉の除去】\n"
    "入力: 「壁紙の剥がれ直ってないんだけど！何回言わせるの？ふざけんな！今日中に見に来い！」\n"
    '出力: { "aggressionScore": 0.9, "converted": "【施工品質・不具合】壁紙の補修が未完了とのご指摘。至急の対応（本日中の訪問）をご要望。" }\n'  # 暴言を除去し，要求を抽出する（カテゴリ付与）
    "\n"
    "【変換例3：正の感情・感謝（原文尊重）】\n"
    "入力: 「先日は急なお願いにも関わらず、すぐに網戸を直してくださり本当に助かりました。職人さんにもよろしくお伝えください。」\n"
    '出力: { "aggressionScore": 0.0, "converted": "【施工品質・不具合】先日は急なお願いにも関わらず、すぐに網戸を直してくださり本当に助かりました。職人さんにもよろしくお伝えください。" }\n'  # 感謝の言葉は原文を尊重する（カテゴリ付与）
)  # 具体例の定義をここで閉じる
