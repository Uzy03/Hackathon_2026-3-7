...
## 3. 工務店（AI）返信（RAG）
- 工務店（AI）の返信案（replySuggestion）は Gemini が生成します。
- Supabase に登録した「工務店固有の知識（営業時間、方針、法律対応の注意点など）」を検索し、プロンプトに文脈として注入します（RAG）。
- 知識に無いことは無理に断定せず、「確認が必要」と伝える方針です。
...
- AI Engine: Gemini API（毒抜き + 攻撃性スコア + 返信案生成）

- DB: Supabase（customers/messages/knowledge）
...
## 画面（ポート分離）
### ローカル開発（ポート分離）
- 工務店ダッシュボード: `http://localhost:3000`（admin）
- 工務店チャット（クライアント入力）: `http://localhost:3001`（client）
...
```mermaid
flowchart LR
  Browser["ブラウザ"] -->|HTTPS| FE["Next.js (Vercel) /admin /client"]
  FE -->|/backend-api/*| Proxy["Next Route Handler (proxy)"]
  Proxy -->|HTTPS| API["FastAPI (Render) /api/*"]
  API -->|Embedding| GMEmb["Gemini Embeddings"]
  API -->|Generate| GM["Gemini API"]
  API -->|DB| DB["Supabase (customers/messages/knowledge)"]
```
...
- [x] Phase 4: 工務店（AI）返信を RAG（知識参照）付きで生成へ置換
