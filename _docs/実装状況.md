...
flowchart LR
  ...
  FE -->|rewrite: /api/:path* ⇒ http://localhost:8000/api/:path*| BE["FastAPI + Gemini + Supabase"]
  BE -->|JSON response| FE
...
- バックエンド: FastAPI（Python）で `/api/convert` を提供（Gemini による実変換 + Supabase 永続化）
...
- エンドポイント: `POST /api/convert`, `GET /api/messages`, `GET /api/stats/customers`
...
### 履歴表示（DB から取得）
- 初期ロードで `GET /api/messages?session_id=...` を呼び、履歴を表示: [MessageList.tsx](file:///Users/ujihara/ハッカソン_2026-3-7/frontend/src/features/dashboard/components/MessageList.tsx)
