# 実装状況（2026-03-21 時点）

このドキュメントは、現在のリポジトリの実装状態を、構成・接続・技術要素が分かるように整理したものです。

## 全体像（アーキテクチャ）

```mermaid
flowchart LR
  U["ユーザー（ブラウザ）"] -->|HTTPS| FE["Next.js (Vercel) /admin /client"]
  FE -->|/backend-api/*| Proxy["Next Route Handler (proxy)"]
  Proxy -->|HTTPS| BE["FastAPI (Render) /api/*"]
  BE -->|Embedding| GMEmb["Gemini Embeddings"]
  BE -->|Gemini| GM["Gemini API"]
  BE -->|DB read/write| DB["Supabase DB (customers/messages/knowledge)"]
```

- フロントエンド: Next.js（App Router）でUIを表示
- クレーマー画面: `/client`（入力 + 工務店（AI）返信 + マスコット）
- 工務店画面: `/admin`（顧客一覧 + 顧客詳細/履歴）
- バックエンド: FastAPI（Python）で `/api/*` を提供（Gemini 解析 + Supabase 永続化）
- 接続: フロントからは相対 `/backend-api/*` を呼び、Next.js Route Handler が Render の FastAPI へプロキシする

## 画面/データフロー（メッセージ送信）

```mermaid
sequenceDiagram
  participant User as User (Browser)
  participant Page as Next.js /client
  participant Chat as ClientChatComposer/useChatInput
  participant Storage as localStorage
  participant Next as Next.js Route Handler (/backend-api/*)
  participant API as FastAPI (Render) /api/convert
  participant Emb as Gemini Embeddings
  participant RPC as Supabase RPC match_knowledge
  participant Gemini as Gemini API (Flash)
  participant DB as Supabase DB

  User->>Chat: クレーマー文を入力・送信
  Chat->>Storage: session_id を取得（無ければ生成して保存）
  Chat->>Next: POST /backend-api/convert (session_id + message)
  Next->>API: POST https://<service>.onrender.com/api/convert (session_id + message)
  API->>DB: customers を検索/作成して customer_id を確定
  API->>Emb: 入力文を埋め込み（768次元）へ変換
  API->>RPC: 関連知識を上位N件取得
  RPC-->>API: knowledge（content + similarity）
  API->>Gemini: 毒抜き + 攻撃性スコア + 返信案（知識コンテキストを注入）
  Gemini-->>API: aggressionScore, converted, replySuggestion
  API->>DB: messages に保存（original_text=生文, converted_text=JSON文字列）
  API-->>Next: original, converted, aggressionScore, replySuggestion
  Next-->>Chat: JSONレスポンス
  Chat-->>Page: チャットに追加、マスコット更新
```

### メッセージ保存/表示の意味（現状）
- DB（messages）
  - `original_text`: クレーマーの生文
  - `converted_text`: JSON文字列（毒抜き文 + 返信案）
- client 画面（/client）
  - クライアント入力（生文）と工務店（AI）返信を表示
  - 毒抜き結果そのものは表示しない
- admin 画面（/admin）
  - 「クライアント毒抜き文」＋「工務店（AI）返信」のみを表示
  - クライアントの生文は表示しない

## 画面/データフロー（工務店：顧客選択）

```mermaid
sequenceDiagram
  participant Admin as Admin (Browser)
  participant Page as Next.js /admin
  participant Next as Next.js Route Handler (/backend-api/*)
  participant API as FastAPI (Render) /api/customers

  Admin->>Page: /admin を開く
  Page->>Next: GET /backend-api/customers
  Next->>API: GET https://<service>.onrender.com/api/customers
  API-->>Next: customers + stats
  Next-->>Page: 顧客一覧を表示
```

## ディレクトリツリー（主要部）

※ `node_modules/` や `.next/` 等の生成物は除外して記載。

```text
.
├── AGENT.md
├── README.md
├── package.json
├── package-lock.json
├── .gitignore
├── _docs/
│   ├── アプリケーション全体像.md
│   └── 実装状況.md  ← このファイル
├── knowledge.txt  # RAG用の知識ソース（手動編集→同期投入）
├── backend/
│   ├── main.py
│   ├── requirements.txt
│   ├── .env.example
│   ├── scripts/
│   │   └── ingest_knowledge.py
│   └── src/
│       ├── __init__.py
│       ├── schema.py
│       ├── database.py
│       └── gemini_engine.py
├── frontend/
│   ├── next.config.ts
│   ├── package.json
│   ├── package-lock.json
│   ├── src/
│   │   ├── app/
│   │   │   ├── page.tsx  （/admin と /client の導線）
│   │   │   ├── admin/page.tsx
│   │   │   ├── client/page.tsx
│   │   │   ├── api/[...path]/route.ts  （互換用プロキシ）
│   │   │   └── backend-api/[...path]/route.ts  （本番用プロキシ）
│   │   │   ├── layout.tsx
│   │   │   ├── globals.css
│   │   │   └── favicon.ico
│   │   ├── features/
│   │   │   ├── chat/
│   │   │   │   ├── components/ChatForm.tsx
│   │   │   │   └── hooks/useChatInput.ts
│   │   │   ├── client/
│   │   │   │   ├── components/ClientChatPage.tsx
│   │   │   │   ├── components/ClientChatTimeline.tsx
│   │   │   │   ├── components/ClientChatComposer.tsx
│   │   │   │   └── hooks/useClientChatState.ts
│   │   │   ├── admin/
│   │   │   │   ├── components/AdminDashboardPage.tsx
│   │   │   │   ├── components/CustomerListPanel.tsx
│   │   │   │   ├── components/CustomerDetailPanel.tsx
│   │   │   │   ├── components/CustomerMessageTimeline.tsx
│   │   │   │   ├── hooks/useCustomers.ts
│   │   │   │   ├── hooks/useCustomerMessages.ts
│   │   │   │   └── types.ts
│   │   │   ├── session/
│   │   │   │   └── hooks/useSessionId.ts
│   │   │   ├── mascot/
│   │   │   │   ├── components/DebugSlider.tsx
│   │   │   │   ├── components/MascotDisplay.tsx
│   │   │   │   └── hooks/useMascotState.ts
│   │   │   └── dashboard/
│   │   │       └── components/MessageList.tsx
│   │   └── types/index.ts
│   └── public/...
└── infrastructure/
    └── README.md
```

## 技術スタック（現状の実装）

### Frontend
- Next.js 16.1.6（Turbopack dev server）: [next.config.ts](file:///Users/ujihara/ハッカソン_2026-3-7/frontend/next.config.ts)
- React 19 / TypeScript
- Tailwind CSS v4（`@import "tailwindcss";`）: [globals.css](file:///Users/ujihara/ハッカソン_2026-3-7/frontend/src/app/globals.css)
- UI補助: `lucide-react`, `clsx`, `tailwind-merge`

### Backend
- FastAPI + Uvicorn（ポート 8000）: [main.py](file:///Users/ujihara/ハッカソン_2026-3-7/backend/main.py)
- Pydantic（入出力スキーマ）: [schema.py](file:///Users/ujihara/ハッカソン_2026-3-7/backend/src/schema.py)
- Gemini 呼び出し: Gemini Developer API（REST, `requests`）: [gemini_engine.py](file:///Users/ujihara/ハッカソン_2026-3-7/backend/src/gemini_engine.py)
- Supabase 永続化: `supabase` SDK（customers/messages/stats/knowledge）: [database.py](file:///Users/ujihara/ハッカソン_2026-3-7/backend/src/database.py)
- エンドポイント: `POST /api/convert`, `GET /api/messages`, `GET /api/stats/customers`, `GET /api/customers`, `GET /api/customers/{customer_id}/messages`

### Dev Scripts（ルート）
ルートの `package.json` で「ルートで完結」を実現しています: [package.json](file:///Users/ujihara/ハッカソン_2026-3-7/package.json)

- `npm install`
  - `postinstall` により `frontend` の `npm install` を実行
- `npm run dev`
  - `predev` で 3000/3001/8000 のプロセスを掃除（macOS向け）
  - `concurrently` で admin(3000)/client(3001)/backend(8000) を同時起動
  - backend 起動は `uvicorn main:app --reload --port 8000`（`.venv` があれば activate を試行）
- `npm run dev:admin`
  - 工務店画面（3000）+ backend（8000）
- `npm run dev:client`
  - クレーマー画面（3001）+ backend（8000）
- `npm run knowledge`
  - `knowledge.txt` を読み込み、Supabase の `knowledge` テーブルへ同期（全削除→入れ直し）
- `npm run knowledge:dry`
  - 同期せず、カテゴリ別件数と差分（insert/update/skip）を表示

## Frontend 実装詳細

### 画面構成
- 3000: 工務店画面（admin専用）
- 3001: クレーマー画面（client専用）
- クレーマー画面: [client/page.tsx](file:///Users/ujihara/ハッカソン_2026-3-7/frontend/src/app/client/page.tsx)
  - タイトルは「工務店チャット」
  - LINE 風チャット（クライアント入力 / 工務店（AI）返信）
  - 送信はボタンまたは Ctrl+Enter / Cmd+Enter
  - 入力欄は画面下に固定（履歴のみスクロール）
  - 画面は「チャット:マスコット=7:3」で分割し、マスコットは画像のみ表示
- 工務店画面: [admin/page.tsx](file:///Users/ujihara/ハッカソン_2026-3-7/frontend/src/app/admin/page.tsx)
  - 顧客一覧（ID + 暫定名 + 統計）
  - 顧客詳細（履歴）
  - 左（顧客一覧）と右（顧客詳細）は独立スクロール

```mermaid
flowchart TB
  Root[app/page.tsx] --> Client[app/client/page.tsx]
  Root --> Admin[app/admin/page.tsx]
  Client --> ClientUI[features/client/*]
  Admin --> AdminUI[features/admin/*]
  ClientUI -->|fetch /backend-api/*| APIProxy[Next Route Handler]
  AdminUI -->|fetch /backend-api/*| APIProxy
```

### 状態管理（現状）
- `/client` は `useClientChatState` が履歴と新規送信を統合して管理
- `/admin` は顧客一覧の選択状態と、選択顧客の履歴をローカル state で管理

### セッション管理（顧客識別）
- ブラウザ単位で `session_id` を生成・保持（localStorage）: [useSessionId.ts](file:///Users/ujihara/ハッカソン_2026-3-7/frontend/src/features/session/hooks/useSessionId.ts)
- `POST /backend-api/convert` の body に `session_id` を同梱: [useChatInput.ts](file:///Users/ujihara/ハッカソン_2026-3-7/frontend/src/features/chat/hooks/useChatInput.ts)

### 履歴表示（DB から取得）
- `/client`: 初期ロードで `GET /backend-api/messages?session_id=...` を呼び、チャット履歴として表示: [useClientChatState.ts](file:///Users/ujihara/ハッカソン_2026-3-7/frontend/src/features/client/hooks/useClientChatState.ts)
- `/admin`: 顧客選択後に `GET /backend-api/customers/{customer_id}/messages` を呼び、履歴として表示: [useCustomerMessages.ts](file:///Users/ujihara/ハッカソン_2026-3-7/frontend/src/features/admin/hooks/useCustomerMessages.ts)

## RAG（知識検索）実装概要

- Supabase に `knowledge` テーブル（`content`, `embedding vector(768)`, `category`）を用意し、`match_knowledge` RPC で類似検索する
- Backend（`POST /api/convert`）で「入力文→Embedding→RPC検索→プロンプトへ注入→返信案生成」を行う
- 知識投入は `knowledge.txt` を手動編集し、`npm run knowledge` で同期する

### 型定義
共通型は [types/index.ts](file:///Users/ujihara/ハッカソン_2026-3-7/frontend/src/types/index.ts) に定義。

|型|用途|
|---|---|
|`Message`|original/converted/timestamp/aggressionScore を保持|
|`Customer`|customers の基本情報（現状は型のみ、UI未使用）|
|`CustomerStats`|顧客別統計（平均スコア/件数/最終日時）|
|`AppState`|将来のContext化に備えた形（現状は未使用）|

## Backend 実装詳細（FastAPI + Gemini + Supabase）

### API: POST /api/convert
実装: [main.py](file:///Users/ujihara/ハッカソン_2026-3-7/backend/main.py)

- 入力: `{ "session_id": string, "message": string }`
- 出力: `{ "original": string, "converted": string, "aggressionScore": float }`
- 動作:
  - `session_id` から顧客を識別（customers を検索し、無ければ作成）
  - `GeminiEngine` が Gemini に「毒抜き変換」と「攻撃性スコア算出」を同時に依頼
  - Gemini の返答は JSON（`{ aggressionScore, converted }`）を厳格に要求し、Pydantic で検証
  - `aggressionScore` は 0.0〜1.0 にクランプして UI 整合性を担保
  - DB へ保存（`original_text`=毒抜きクライアント文、`converted_text`=工務店（AI）返信）
  - client への返却は「工務店（AI）返信」（MVP: 定型文）
  - `GEMINI_API_KEY` 未設定の場合は 503 を返して「設定不足」を明示

### API: GET /api/messages
実装: [main.py](file:///Users/ujihara/ハッカソン_2026-3-7/backend/main.py)

- 入力: `session_id`（query）
- 出力: messages の配列（新しい順）
- 動作:
  - `session_id` から `customer_id` を確定し、messages を取得して返す

### API: GET /api/stats/customers
実装: [main.py](file:///Users/ujihara/ハッカソン_2026-3-7/backend/main.py)

- 出力: 顧客別統計（`avgAggressionScore`, `messageCount`, `lastMessageAt`）
- 動作:
  - MVP として messages を取得し、Python 側で集計して返す: [database.py](file:///Users/ujihara/ハッカソン_2026-3-7/backend/src/database.py)

### API: GET /api/customers
実装: [main.py](file:///Users/ujihara/ハッカソン_2026-3-7/backend/main.py)

- 出力: 顧客一覧（`customerId`, `displayName`, `avgAggressionScore`, `messageCount`, `lastMessageAt`, `createdAt`）
- 動作:
  - customers 一覧と messages 統計を合成して返す: [database.py](file:///Users/ujihara/ハッカソン_2026-3-7/backend/src/database.py)

### API: GET /api/customers/{customer_id}/messages
実装: [main.py](file:///Users/ujihara/ハッカソン_2026-3-7/backend/main.py)

- 出力: 指定顧客の messages 配列（新しい順）
- 動作:
  - `customer_id` で messages を取得して返す: [database.py](file:///Users/ujihara/ハッカソン_2026-3-7/backend/src/database.py)

### 環境変数
- `backend/.env`（ローカル用、Git 管理外）
  - `GEMINI_API_KEY`: Google AI Studio の API キー
  - `GEMINI_MODEL`: 使用モデル（未指定時はデフォルト）
  - `SUPABASE_URL`: Supabase プロジェクト URL
  - `SUPABASE_ANON_KEY`: Supabase anon key
- サンプル: [backend/.env.example](file:///Users/ujihara/ハッカソン_2026-3-7/backend/.env.example)

### 使用モデル（現状）
- デフォルト: `models/gemini-flash-latest`
- 環境変数 `GEMINI_MODEL` で上書き可能
- 実装: [GeminiEngine](file:///Users/ujihara/ハッカソン_2026-3-7/backend/src/gemini_engine.py)

#### モデル名に関する注意
- キー/プロジェクトによって利用可能なモデル名が異なり、存在しないモデル名を指定すると `/api/convert` が 500 になります（Gemini 側が 404 を返す）。
- その場合は `GEMINI_MODEL` を `models/gemini-flash-latest` など「ListModels で確認できる名前」に変更します。
- 参考: `models/gemini-2.5-flash` / `models/gemini-2.0-flash` / `models/gemini-3-flash-preview` などが見える環境があります（利用可否はアカウント/リージョン/ティアに依存）。

## Frontend ↔ Backend の接続状況

### 現状の接続方式（推奨）
- フロントの fetch は相対パス `/backend-api/*` を使用（`/backend-api/convert`, `/backend-api/messages` など）
  - 送信: [useChatInput.ts](file:///Users/ujihara/ハッカソン_2026-3-7/frontend/src/features/chat/hooks/useChatInput.ts)
  - 履歴（client）: [useClientChatState.ts](file:///Users/ujihara/ハッカソン_2026-3-7/frontend/src/features/client/hooks/useClientChatState.ts)
  - 履歴（admin）: [useCustomerMessages.ts](file:///Users/ujihara/ハッカソン_2026-3-7/frontend/src/features/admin/hooks/useCustomerMessages.ts)
- Next.js Route Handler が Render の FastAPI へプロキシ: [backend-api route](file:///Users/ujihara/ハッカソン_2026-3-7/frontend/src/app/backend-api/%5B...path%5D/route.ts)

この方式のメリット:
- ブラウザは Vercel へ同一オリジンでアクセスでき、バックエンドの URL/制約差分の影響を受けにくい

## 運用上の注意 / 既知の課題

### 1) Turbopack の「複数 lockfile」警告
ルートと `frontend/` の両方に `package-lock.json` が存在するため、Next.js がワークスペースのルート推測で警告を出すことがあります。
- 現状は警告のみで動作するが、将来的には `turbopack.root` 設定で明示する選択肢があります。

### 2) `frontend/src/features/mascot/components/MascotDisplay.tsx` の震え実装
現在、震え表現は CSS アニメーションで実装しています（レンダー中に `Math.random()` を呼ばない）:
- 実装: [MascotDisplay.tsx](file:///Users/ujihara/ハッカソン_2026-3-7/frontend/src/features/mascot/components/MascotDisplay.tsx)
- アニメーション定義: [globals.css](file:///Users/ujihara/ハッカソン_2026-3-7/frontend/src/app/globals.css)

### 3) README と実装の差分
このリポジトリの「構想」と「実装」がズレないよう、以下 2 つを現状に合わせて更新しています。
- 現状の全体像: [_docs/アプリケーション全体像.md](file:///Users/ujihara/ハッカソン_2026-3-7/_docs/アプリケーション全体像.md)
- 現状の詳細: [README.md](file:///Users/ujihara/ハッカソン_2026-3-7/README.md)

## 次の実装候補（ロードマップ案）

```mermaid
flowchart TD
  A[現状: FastAPI + Gemini + Supabase] --> B[変換品質: 要求抽出の精度改善]
  A --> C[構造化出力: JSON Schema / リトライ戦略]
  A --> D[攻撃性スコア: 判定基準の改善・安定化]
  A --> E[統計: SQL View / RPC で高速化]
  A --> F[状態管理: Context/Store]
  A --> G[UI演出: アニメーション最適化]
  A --> H[デプロイ: Cloud Run など]
```
